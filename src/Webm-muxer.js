...
class WebmMuxer {
   ...
   * the muxer will write a new cluster. default to 300 if bufferSize is null
   *
  ...
  addFrameFromBlob(frame, array) {
    ...
    const keyframe = this.#frameCount === 0;
    ...
    if (this.#frameCount === 0 || this.#frameCount >= this.#bufferSize) {
      this.#blockChunks = [];
      this.#clusterTimeCode = timestampMs;
      this.makeClusterStart(this.#clusterTimeCode, array);
    }

    this.makeSimpleBlock(frame.subarray(cursor + 8), timestampMs - this.#clusterTimeCode, keyframe, array);
